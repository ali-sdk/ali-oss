...
proto.putBucket = function* putBucket(name, region, options) {
  ...
  if (region) {
    ...
    if (!options.StorageClass) {
      ...
      params.content = `${'<?xml version="1.0" encoding="UTF-8"?>\n' +
        '<CreateBucketConfiguration><LocationConstraint>'}${region}</LocationConstraint><StorageClass>${options.StorageClass}</StorageClass></CreateBucketConfiguration>`;
    }
  } else if (options.StorageClass) {
    params.mime = 'xml';
    params.content = `${'<?xml version="1.0" encoding="UTF-8"?>\n' +

    ...
    return new Promise(async (resolve, reject) => {
      try {
        if (!self.isCancel()) {
          ...
          const data = {
            stream: await self._createStream(file, pi.start, pi.end, true),
...
proto._createStream = async function _createStream(file, start, end, useMd5) {
  const streamInfo = {};
  let fileContent;
  if (isBlob(file) || isFile(file)) {
    fileContent = file.slice(start, end);
    fileContent = await fileContent.arrayBuffer();
    streamInfo.stream = new WebFileReadStream(file.slice(start, end));
  } else if (isBuffer(file)) {
    // we can't use Readable.from() since it is only support in Node v10
    const iterable = file.subarray(start, end);
    fileContent = iterable;
    streamInfo.stream = new Readable({
      read() {
        this.push(iterable);
        this.push(null);
      }

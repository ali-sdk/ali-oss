...
const deepCopy = require('../common/utils/deepCopy');
...
proto.multipartUpload = async function multipartUpload(name, file, options) {
  ...
  const initOptions = deepCopy(options);
  delete opts.headers['x-oss-server-side-encryption'];

  if (opts.checkpoint && opts.checkpoint.uploadId) {
    return await this._resumeMultipart(opts.checkpoint, opts);
  }
  ...
  if (!opts.mime) {
    if (is.file(file)) {
      opts.mime = mime.getType(path.extname(file.name));
    } else if (is.blob(file)) {
      opts.mime = file.type;
    } else {
      opts.mime = mime.getType(path.extname(file));
    }
  }

  opts.headers = opts.headers || {};
  this._convertMetaToHeaders(opts.meta, opts.headers);

  const fileSize = await this._getFileSize(file);
  if (fileSize < minPartSize) {
    const stream = this._createStream(file, 0, fileSize);
    opts.contentLength = fileSize;

    const result = await this.putStream(name, stream, opts);
    if (opts && opts.progress) {
      await opts.progress(1);
    }
    ...
    if ((opts.headers && opts.headers['x-oss-callback']) || opts.callback) {
      ret.data = result.data;
    }

    return ret;
  }
  if (opts.partSize && !(parseInt(opts.partSize, 10) === opts.partSize)) {
    throw new Error('partSize must be int number');
  }

  if (opts.partSize && opts.partSize < minPartSize) {
    throw new Error(`partSize must not be smaller than ${minPartSize}`);
  }

  const initResult = await this.initMultipartUpload(name, initOptions);
  const { uploadId } = initResult;
  const partSize = this._getPartSize(fileSize, opts.partSize);
  ...
  if (opts && opts.progress) {
    await opts.progress(0, checkpoint, initResult.res);
  }


  return await this._resumeMultipart(checkpoint, opts);
};

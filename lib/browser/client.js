...
proto.request = function* request(params) {
  ...
  if (reqErr || params.successStatuses && params.successStatuses.indexOf(result.status) === -1) {
    var err = yield* this.requestError(result, reqErr);
...
proto.requestError = function* requestError(result, reqError) {
  if (reqError) {
      return reqError;
  }
  var err;

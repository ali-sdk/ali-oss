...
proto.signature = function signature(stringToSign) {
  this.debug('authorization stringToSign: %s', stringToSign);
...
proto.createRequest = function createRequest(params) {
  ...
  this.debug('request %s %s, with headers %j, !!stream: %s', params.method, url, headers, !!params.stream);
...
proto.request = async function request(params) {
  ...
  try {
    ...
    this.debug('response %s %s, got %s, headers: %j', params.method, reqParams.url, result.status, result.headers);
  } catch (err) {
    reqErr = err;
  }
  let err;
...
proto.requestError = async function requestError(result) {
  ...
  if (!result.data || !result.data.length) {
    ...
    this.debug('request response error data: %s', message);

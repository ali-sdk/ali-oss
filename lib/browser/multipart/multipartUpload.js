...
async function multipartUpload(name, file, options = {}) {
    if (this.options.multipartRunning) {
        console.warn('multipartUpload is already executing, if you need to call more than one multipartUpload at the same time, please create more instances');
    }
    ...
    if (fileSize < minPartSize) {
        ...
        const result = await put_1.put.call(this, name, file, options);

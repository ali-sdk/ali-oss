...
proto.multipartUpload = function* multipartUpload(name, file, options) {
  ...
  const fileSize = yield this._getFileSize(file);
  if (fileSize < minPartSize) {
    const stream = this._createStream(file, 0, fileSize);
    options.contentLength = fileSize;

    const result = yield this.putStream(name, stream, options);
  ...
  const initResult = yield this.initMultipartUpload(name, options);
  ...
  if (options && options.progress) {
    yield options.progress(0, checkpoint, initResult.res);
  }
...
proto._resumeMultipart = function* _resumeMultipart(checkpoint, options) {
  ...
  const multipartStatus = { finish: false };
  ...
  const uploadPartJob = function* uploadPartJob(self, partNo) {
    if (!self.isCancel()) {
      try {
        ...
        if (!self.isCancel() && !multipartStatus.finish) {

 ...
 * @param {Objeat} options
 * @param {res}
 */

proto.putTagging = async function putTagging(name, taggings, options) {
  if (!Array.isArray(taggings)) {
    throw new Error('Please use an array like [{Key-Value}]');
  }
  options = options || {};
  let xml = '<?xml version="1.0" encoding="UTF-8"?>\n<Tagging>\n  <TagSet>\n';
  if (Array.isArray(taggings) && taggings.length < 11) {
    for (let i = 0; i < taggings.length; i++) { 
      for (let key in taggings[i]) {
        xml += `  <Tag>\n    <Key>${key}</Key>\n    <Value>${taggings[i][key]}</Value>\n  </Tag>\n`;
      }
    }
    xml += '  </TagSet>\n</Tagging>';
  } else {
    throw new Error('Associate up to ten tags')
  }
 ...
 * @param {res}
 */
proto.getTagging = async function getTagging(name, options) {
  options = options || {};
  options.subres = 'tagging';
  name = this._objectName(name);
  const params = this._objectRequestParams('GET', name, options);
  ...
  params.xmlResponse = true;
  const result = await this.request(params);
  const tag = result.data.TagSet.Tag;
  let tags = tag.map(ele => {
    return {
      [ele.Key]: ele.Value
    } 
  })
...
proto.deleteTagging = async function deleteTagging(name, options) {
  options = options || {};
  options.subres = 'tagging';
  const params = this._objectRequestParams('DELETE', name, options);
  params.successStatuses = [204];

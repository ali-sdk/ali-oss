...
const crc64 = require('./common/utils/checkCrc64');
 ...
 *                    key2: 'value2'
 *                  }
 * @param {Boolean} crc64 default value is false
 * @return {Object}
 */
proto.put = async function put(name, file, options) {
  ...
  if (options.crc64) {
    if (!crc64.checkCrc64(content, result.res.headers['x-oss-hash-crc64ecma'])) {
...
proto.putStream = async function putStream(name, stream, options) {
  ...
  const result = await this.request(params);
  if (options.crc64) {
    crc64.checkCrc64Stream(stream.path, (err, data) => {
      if (!err) {

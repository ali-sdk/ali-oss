...
proto.putSymlink = async function(objectName, sourceName, options = {}){
  const _sourceName = encodeURIComponent(sourceName)

  options.headers = options.headers || {};
  this._convertMetaToHeaders(options.meta, options.headers);
  options.headers['x-oss-symlink-target'] = _sourceName
  ...
  const params = this._objectRequestParams(method, objectName, options);

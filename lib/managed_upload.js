...
proto.multipartUpload = async function multipartUpload(name, file, options) {
  ...
  if (options.checkpoint && options.checkpoint.uploadId) {
    const result = await this._resumeMultipart(options.checkpoint, options);
    return result;
  }
  ...
  const result = await this._resumeMultipart(checkpoint, options);
  return result;
};
...
proto._resumeMultipart = async function _resumeMultipart(checkpoint, options) {
  ...
  if (this.checkBrowserAndVersion('Internet Explorer', '10') || parallel === 1) {
    ...
    // // upload in parallel
    const jobErr = await this._parallel(todo, parallel, uploadPartJob);

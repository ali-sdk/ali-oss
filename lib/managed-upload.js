 ...
 *                  }
 */
proto.multipartUpload = async function multipartUpload(name, file, _options) {
  this.resetCancelFlag();
  const options = JSON.parse(JSON.stringify(_options || {}));
  const initOptions = JSON.parse(JSON.stringify(options || {}));
  delete options.headers['x-oss-server-side-encryption'];

...
proto.putBucketLifecycle = function* putBucketLifecycle(bucket, rules, options) {
  // rules: [rule, ...]
  // rule: {[id], prefix, status, [expiration], [transition], [abortMultipartUpload]}
  // rule.status: 'Enabled' or 'Disabled'
  // rule.expiration: {days or createdBeforeDate}
  // rule.transition: [{days or createdBeforeDate, storageClass}]
  // rule.abortMultipartUpload: {days or createdBeforeDate}
  ...
  for (var i = 0; i < rules.length; i++) {
    ...
    if (rule.expiration) {
      xmls.push('    <Expiration>\n')
      xmls.push('      ' + dateToXML(rule.expiration) + '\n');
      xmls.push('    </Expiration>\n');

    }
    if (rule.transition && rule.transition.length <= 2) {
      rule.transition.forEach(function(item) {
        xmls.push('    <Transition>\n')
        xmls.push('      ' + dateToXML(item) + '\n');
        xmls.push('      <StorageClass>' + item.storageClass + '</StorageClass>\n');
        xmls.push('    </Transition>\n');
      })
    }
    if (rule.abortMultipartUpload) {
      xmls.push('    <AbortMultipartUpload>\n')
      xmls.push('      ' + dateToXML(rule.abortMultipartUpload) + '\n');
      xmls.push('    </AbortMultipartUpload>\n');
    }
    xmls.push('  </Rule>\n');
  }
  xmls.push('</LifecycleConfiguration>')
  params.content = xmls.join('');
  params.mime = 'xml';
  params.successStatuses = [200];
  var result = yield this.request(params);
  return {
    ...
    rules = rules.map(function (rule) {
      ...
      if (rule.Expiration) {
        ...
        if (rule.Expiration.Days) {
          item.expiration.days = +rule.Expiration.Days;
        } else {
          item.expiration.createdBeforeDate = rule.Expiration.CreatedBeforeDate;
        }
      }
      if (rule.Transition) {
        if (!isArray(rule.Transition)) {
          rule.Transition = [rule.Transition];
        }
        item.transition = [];
        rule.Transition.forEach(function(val) {
          var transition = {}
          if (val.Days) {
            transition.days = +val.Days;
          } else {
            transition.createdBeforeDate = val.CreatedBeforeDate;
          }
          transition.storageClass = val.StorageClass;
          item.transition.push(transition);
        })
      }
      if (rule.AbortMultipartUpload) {
        item.abortMultipartUpload = {};
        if (rule.AbortMultipartUpload.Days) {
          item.abortMultipartUpload.days = +rule.AbortMultipartUpload.Days;
        } else {
          item.abortMultipartUpload.createdBeforeDate = rule.AbortMultipartUpload.CreatedBeforeDate;
        }
      }
      return item;
    });
...
function dateToXML(obj) {
  if (obj.days) {
    return '<Days>' + obj.days + '</Days>'
  } else if (obj.createdBeforeDate) {
    return '<CreatedBeforeDate>' + obj.createdBeforeDate + '</CreatedBeforeDate>'
  }
}

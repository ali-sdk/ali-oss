...
proto.putBucketCORS = function* (name, region, options) {
  options = options || {};
  var rules = options.rules || [];
  assert(rules.length, 'options.rules is required');
  rules.forEach(rule => {
  ...
  for (var i = 0, l = rules.length; i < l; i++) {
    ...
    toArray(rule.maxAgeSeconds).forEach(function(val) {
      xml += '<MaxAgeSeconds>' + val + '</MaxAgeSeconds>';
    });
    xml += '</CORSRule>';
  }

...
function default_1(OssClient) {
    function Client(options) {
        ...
        sdk_base_1.default.call(this);
        this.clients = [];
        this.availables = {};
        for (let i = 0; i < options.cluster.length; i++) {
            const opt = options.cluster[i];
            copy_to_1.default(options).pick('timeout', 'agent', 'urllib').to(opt);
    ...
    util_1.default.inherits(Client, sdk_base_1.default);
    const proto = Client.prototype;
    get_ready_1.default.mixin(proto);
    ...
    proto._checkAvailable = async function _checkAvailable(ignoreStatusFile) {
        ...
        for (let i = 0; i < this.clients.length; i++) {
            ...
            if (!available) {
                ...
                available = await this._checkStatus(client, name);
                if (!available) {
                    downStatusFiles.push(objectUrl_1.objectUrl.call(client, name));
                }
            }

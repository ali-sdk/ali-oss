...
async function multipartDownload(objectKey, filePath, options = {}) {
    ...
    options = Object.assign({
        ...
        enableCRC64: options.enableCRC64 && crc64_1.SUPPORT_BIGINT
    });
    if (originEnableCRC64 && !crc64_1.SUPPORT_BIGINT && !options.disabledWarning) {
    ...
    const download = () => {
            ...
            return new Promise((resolve, reject) => {
                ...
                readStream.on('close', () => {
                    if (paused) {
                        ...
                        part.status = EDownloadStatus.READY;
                        downloadCheckpoint.dump();
                        reject(_makeCancelEvent_1._makeCancelEvent());
                    }
                });
        ...
        function scheduler(resolve, reject) {
            for (; pendWorkerCount < options.parallel && performNextWorkPart(); pendWorkerCount++) {
                ...
                downloadNextPart().then((partResp) => {
                    if (!partResp)
                        return;
                    pendWorkerCount -= 1;

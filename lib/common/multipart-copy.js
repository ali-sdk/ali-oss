...
proto.multipartUploadCopy = async function multipartUploadCopy(name, sourceData, options) {
  ...
  if (options.checkpoint && options.checkpoint.uploadId) {
    const result = await this._resumeMultipartCopy(options.checkpoint, sourceData, options);
    return result;
  }
  ...
  const result = await this._resumeMultipartCopy(checkpoint, sourceData, options);
  return result;
};
...
proto._resumeMultipartCopy = async function _resumeMultipartCopy(checkpoint, sourceData, options) {
  ...
  if (this.checkBrowserAndVersion('Internet Explorer', '10') || parallel === 1) {
    ...
    // upload in parallel
    const errors = await this._parallelCopy(todo, parallel, uploadPartJob, sourceData);
  ...
  const result = await this.completeMultipartUpload(name, uploadId, doneParts, options);
  return result;
};

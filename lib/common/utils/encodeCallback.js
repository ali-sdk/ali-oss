...
function encodeCallback(reqParams, options) {
    ...
    if (!Object.prototype.hasOwnProperty.call(reqParams.headers, 'x-oss-callback')) {
        if (options.callback) {
            ...
            const callback = buffer_1.Buffer.from(JSON.stringify(json)).toString('base64');
            ...
            if (options.callback.customValue) {
                ...
                reqParams.headers['x-oss-callback-var'] = buffer_1.Buffer.from(JSON.stringify(callbackVar)).toString('base64');
            }
        }
    }
}

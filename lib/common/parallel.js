...
proto._parallel = async function _parallel(todo, parallel, cb) {
  const result = await this._parallelCopy(todo, parallel, cb, null);
  return result;
};

proto._parallelCopy = async function _parallelCopy(todo, parallel, cb, sourceData) {
  const that = this;
  // upload in parallel
  const jobErr = [];
  let jobs = [];
  ...
  for (let i = 0; i < todo.length; i++) {
    ...
    if (sourceData) {
      jobs.push(cb(that, todo[i], sourceData));
    } else {
      jobs.push(cb(that, todo[i]));
    }

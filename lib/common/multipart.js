...
proto.uploadPart = async function uploadPart(name, uploadId, partNo, file, start, end, options) {
  const streamInfo = await this._createStream(file, start, end, true);
  const stream = streamInfo.stream || streamInfo;
  const { md5 } = streamInfo;
  const data = {
    stream,
    size: end - start
  };
  ...
  if (md5) {
    if (options.headers && !options.headers['Content-Md5']) {
...
proto._uploadPart = async function _uploadPart(name, uploadId, partNo, data, options = {}) {
  ...
  const stream = data.stream.stream || data.stream;
  const { md5 } = data.stream;

  if (md5) {
    if (opt.headers && !opt.headers['Content-Md5']) {

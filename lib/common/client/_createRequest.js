...
function _createRequest(params) {
    ...
    if (params.content) {
        if (!headers['Content-MD5'] && !params.disabledMD5) {
            headers['Content-MD5'] = crypto_1.default
                .createHash('md5')
                .update(buffer_1.Buffer.from(params.content, 'utf8'))
                .digest('base64');
        }
    ...
    if (isIP_1.isIP(this.options.endpoint.hostname)) {
        ...
        const hostInfo = initOptions_1.getActualEndpointByRegion(region, internal, secure);

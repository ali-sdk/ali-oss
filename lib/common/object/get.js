...
const { checkCrc64, checkCrc64Stream } = require('../utils/checkCrc64');
...
proto.get = async function get(name, file, options = {}) {
  ...
  try {
    ...
    if (options.crc64 && result.data) {
      if (!checkCrc64(result.data, result.headers['x-oss-hash-crc64ecma'])) {
        throw new Error('crc64 check fail');
      }
    }
    // stream
    if (options.crc64 && writeStream) {
      checkCrc64Stream(writeStream.path, (err, d) => {

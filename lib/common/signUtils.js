...
exports.buildRtmpCanonicalString = function buildRtmpCanonicalString(canonicalizedResource, request, expires) {
  if (is.string(canonicalizedResource)) {
    throw new TypeError('canonicalizedResource\'s type should be String');
  }

  if (is.string(expires)) {
    throw new TypeError('expires\'s type should be String');
  }
  var canonicalString = expires + '\n';

  for (var paramKey in request.parameters) {
    canonicalString += paramKey + ":" + request.parameters[paramKey] + '\n';
  }
  canonicalString += canonicalizedResource;

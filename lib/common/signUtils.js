...
function getProductName(cloudBoxId) {
  if (cloudBoxId === undefined) return 'oss';
  return 'oss-cloudbox';
}
exports.getProductName = getProductName;
function getSignRegion(cloudBoxId, region) {
  if (cloudBoxId === undefined) return region;
  return cloudBoxId;
}
exports.getSignRegion = getSignRegion;
 ...
 * @param {string|undefined} cloudBoxId
 * @returns {string}
 */
exports.getStringToSign = function getStringToSign(region, date, canonicalRequest, cloudBoxId) {
  const stringToSign = [
    'OSS4-HMAC-SHA256',
    date, // TimeStamp
    this.getCredential(date.split('T')[0], region, undefined, getProductName(cloudBoxId)), // Scope
 ...
 * @returns {string}
 */
exports.getSignatureV4 = function getSignatureV4(accessKeySecret, date, region, stringToSign, cloudBoxId) {
  ...
  const signingOss = crypto.createHmac('sha256', signingRegion).update(getProductName(cloudBoxId)).digest();
...
exports.authorizationV4 = function authorizationV4(
  ...
  const stringToSign = this.getStringToSign(region, date, canonicalRequest, cloudBoxId);
  const onlyDate = date.split('T')[0];
  const signatureValue = this.getSignatureV4(accessKeySecret, onlyDate, region, stringToSign, cloudBoxId);
  const additionalHeadersValue =
    fixedAdditionalHeaders.length > 0 ? `AdditionalHeaders=${fixedAdditionalHeaders.join(';')},` : '';

  return `OSS4-HMAC-SHA256 Credential=${this.getCredential(onlyDate, region, accessKeyId, getProductName(cloudBoxId))},${additionalHeadersValue}Signature=${signatureValue}`;
};

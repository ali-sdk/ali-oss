...
little-endian architecture. */
    static void crc64_little_init() { crc64_init(crc64_little_table); }
...
well, initializing and using two tables, if called upon to do so. */
    ...
    std::string V8StringToString(v8::Local<v8::String> str) {
        ...
        v8::String::Utf8Value param1(v8::Isolate::GetCurrent(), str);
    ...
    void crc64(const v8::FunctionCallbackInfo<v8::Value> &args) {
        ...
        v8::Local<v8::Object> bufferObj = args[1]->ToObject(context).ToLocalChecked();
        char *bufferData = node::Buffer::Data(bufferObj);
        size_t bufferLength = node::Buffer::Length(bufferObj);

        auto init_crc = V8StringToInt(args[0]->ToString(context).ToLocalChecked());

        auto value = aos_crc64(init_crc, (void *) bufferData, bufferLength);

        // std::cout<<"init_crc:"<<init_crc<<" buffer:"<<bufferData<<"
        // length:"<<bufferLength<<std::endl;

        auto res = IntToString(value);

        v8::MaybeLocal<v8::String> result = v8::String::NewFromUtf8(isolate, res.c_str());

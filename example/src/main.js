...
const appServer = 'http://127.0.0.1:9000/sts';
...
const progress = function progress(p, checkpoint) {
    if(checkpoint){
        //将当前上传进度保存到localStorage中
        let checkOnlyKey = crypto.createHash('md5').update( (checkpoint.name+checkpoint.file.name+checkpoint.fileSize),"utf-8").digest('hex');
...
const getUploadList = (fileKey,localStorageKey,file)=>{
    return new Promise((resolve,reject)=>{
        uploadFileClient.listUploads({"prefix":fileKey}).then(res=> {
            if (res.nextUploadIdMarker) {
                console.log(localStorageKey)
                let theCheckPoint = JSON.parse(localStorage.getItem(localStorageKey));
                theCheckPoint.file = file;
                resolve(theCheckPoint);
            }else{
                resolve(null)
            }
        }).catch(e=>{
...
const uploadFile =async function uploadFile(client) {
    ...
    let checkOnlyKey = crypto.createHash('md5').update( (key+file.name+file.size),"utf-8").digest('hex');
    try{
        currentCheckpoint = await getUploadList(key,checkOnlyKey,file);
    ...
    return uploadFileClient.multipartUpload(key, file, options).then((res) => {
        ...
        if(currentCheckpoint){
            let theKey = crypto.createHash('md5').update( (currentCheckpoint.name+currentCheckpoint.file.name+currentCheckpoint.file.size),"utf-8").digest('hex');
